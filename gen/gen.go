...
type AutoGen struct {
	FieldIntArray   map[string]xsql.StringArray
	FieldInt64Array map[string]xsql.StringArray
...
func (g *AutoGen) FuncMap() (funcs template.FuncMap) {
	return template.FuncMap{
		...
		"FieldDefineComment": g.FieldDefineComment,
	}
}

func (g *AutoGen) IsIntArrayField(s *Struct, field *Field) (ok bool) {
	if g.FieldIntArray == nil {
		return
	}
	if fields, ok := g.FieldIntArray[s.Table.Name]; ok {
		return fields.HavingOne(field.Column.Name)
	}
	return
}

func (g *AutoGen) IsInt64ArrayField(s *Struct, field *Field) (ok bool) {
	if g.FieldInt64Array == nil {
		return
	}
	if fields, ok := g.FieldInt64Array[s.Table.Name]; ok {
		return fields.HavingOne(field.Column.Name)
	}
	return
}
...
func (g *AutoGen) FieldType(s *Struct, field *Field) (typ string) {
	...
	if g.IsInt64ArrayField(s, field) {
		typ = "xsql.Int64Array"
		return
	}
	if g.IsIntArrayField(s, field) {
		typ = "xsql.IntArray"
		return
	}
	typ = field.Type
...
func (g *AutoGen) FieldDefineType(s *Struct, field *Field) (result string) {
	if g.IsInt64ArrayField(s, field) || g.IsIntArrayField(s, field) {
		result = "Array"
	} else if strings.HasPrefix(field.Type, "*") {
		result = strings.Title(strings.TrimPrefix(field.Type, "*")) + "Ptr"
	} else if strings.HasPrefix(field.Type, "xsql.") {
		result = strings.TrimPrefix(field.Type, "xsql.")
		if result == "M" {
			result = "Object"
		} else if strings.HasSuffix(result, "Array") {
			result = "Array"
		}
	} else if strings.HasPrefix(field.Type, "decimal.") {
		result = strings.TrimPrefix(field.Type, "decimal.")
	} else {
		result = strings.Title(field.Type)
	}
	return
}

func (g *AutoGen) FieldDefineComment(s *Struct, field *Field) (result string) {
...
func (g *AutoGen) OnPre(gen *Gen, table *Table) (data interface{}) {
	...
	for _, field := range s.Fields {
		comments, ok := g.Comments[s.Table.Name]
		if !ok {
			continue
		}
		comment, ok := comments[field.Column.Name]
		if !ok {
			continue
		}
		field.Comment = comment
		field.Column.Comment = comment
	}
		...
		for _, field := range s.Fields {
			...
			case "xsql.M":
				if g.IsInt64ArrayField(s, field) {
					...
					`, arg, field.Name, arg, field.Name)
				} else if g.IsIntArrayField(s, field) {
					defaults += fmt.Sprintf(`
						if len(%v.%v) < 1 {
							%v.%v = xsql.IntArray{}
						}
...
func (g *AutoGen) Generate() (err error) {
	...
	}
	{
		...
		err = ioutil.WriteFile(filepath.Join(g.Out, "auto_models.go"), source, os.ModePerm)
		if err != nil {
			return
		}
	}
	{
		...
		err = ioutil.WriteFile(filepath.Join(g.Out, "auto_define.go"), source, os.ModePerm)
		if err != nil {
			return
		}
	}
	{
		...
		err = ioutil.WriteFile(filepath.Join(g.Out, "auto_func.go"), source, os.ModePerm)
		if err != nil {
			return
		}
	}
	{
		...
		err = ioutil.WriteFile(filepath.Join(g.Out, "auto_func_test.go"), source, os.ModePerm)
		if err != nil {
			return
		}
	}
